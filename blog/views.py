...
def add(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            article.save()
            messages.success(request, "Статья успешно добавлена ^_^")
...
def save(request, id):
    if request.method == "POST":
        ...
        if request.user in getArticle.bookmarks.all():
            ...
            return JsonResponse({"message": "Удалено из закладок"})
        else:
            getArticle.bookmarks.add(request.user)
            return JsonResponse({"message": "Добавлено в закладки"})
    ...
    if request.user.is_authenticated:
        if request.method == "PUT":
            if request.user in targtArticle.upvote.all():
                ...
                return JsonResponse(
                    {"messege": "Голос удалён", "count": targtArticle.upvote.count()}
                )
            else:
                targtArticle.upvote.add(request.user)
                return JsonResponse(
                    {"messege": "Голос добавлен", "count": targtArticle.upvote.count()}
                )
...
def update(request, id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            messages.success(request, "Статья успешно обновлена ^_^")
            return HttpResponseRedirect(reverse("blog:index"))
        else:
            return render(
                request,
        ...
        if request.method == "POST":
            ...
            if content == "":
                return HttpResponseRedirect(
                    reverse("blog:singleArt", kwargs={"id": id})
                )
            newComment = Comment(
                owner=request.user, article=targetArticle, content=content
            )
            newComment.save()
            return JsonResponse(
                {
                    ...
                    "numbComments": Comment.objects.filter(
